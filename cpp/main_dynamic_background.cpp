...
void DynamicBackgroundOpticalFlow::optical_flow_check()
{
	...
	for (int i = 0; i < state.size(); i++)
		if (state[i] != 0)
		{
			...
			if ((x1 < limit_edge_corner || x1 >= gray.cols - limit_edge_corner || x2 < limit_edge_corner || x2 >= gray.cols - limit_edge_corner || y1 < limit_edge_corner || y1 >= gray.rows - limit_edge_corner || y2 < limit_edge_corner || y2 >= gray.rows - limit_edge_corner))
			{
				state[i] = 0;
				continue;
			}
...
bool DynamicBackgroundOpticalFlow::stable_judge()
{
	...
	for (int i = 0; i < state.size(); i++)
		if (state[i])
		{
			if (sqrt((prepoint[i].x - nextpoint[i].x) * (prepoint[i].x - nextpoint[i].x) + (prepoint[i].y - nextpoint[i].y) * (prepoint[i].y - nextpoint[i].y)) < limit_stalbe)
				stable_num++;
		}
				...
				for (int i = 0; i < gray.rows / tt; i++)
					for (int j = 0; j < gray.cols / tt; j++)
					{
						...
						for (int k = 0; k < T.size(); k++)
							if (ROI_mod(T[k].x, T[k].y) && sqrt((T[k].x - y1) * (T[k].x - y1) + (T[k].y - x1) * (T[k].y - x1)) < tt * sqrt(2))
								flag_meiju[i][j]++; //flag_meiju[i][j] += dis[k];
					}
				double mm = 0;
